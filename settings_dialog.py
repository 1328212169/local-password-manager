...
class SettingsDialog(QDialog):
    ...
    def _get_encryption_key(self) -> bytes:
        """生成用于加密邮箱授权码的密钥"""
        # 使用简单的 XOR 加密，密钥基于应用路径生成
        key_material = os.path.abspath(__file__).encode('utf-8')
    ...
    def _encrypt_password(self, password: str) -> str:
        ...
        # XOR 加密
        encrypted = bytes([b ^ key[i % len(key)] for i, b in enumerate(password.encode('utf-8'))])
    ...
    def load_settings(self) -> dict:
        ...
        default_settings = {
            ...
            "enable_auto_lock": True  # 是否启用自动锁定（默认为启用）
        }
            ...
            try:
                with open(self.settings_file, 'r', encoding='utf-8') as f:
                    ...
                    if settings.get("email_password"):
                        settings["email_password"] = self._decrypt_password(settings["email_password"])
                    return settings
            except:
    ...
    def save_settings(self):
        ...
        # 加密邮箱授权码后再保存
        if settings_to_save.get("email_password"):
            settings_to_save["email_password"] = self._encrypt_password(settings_to_save["email_password"])
    ...
    def init_security_tab(self):
        ...
        # 启用自动锁定开关
        self.enable_auto_lock_check = QCheckBox("启用自动锁定")
        self.enable_auto_lock_check.setChecked(self.settings.get("enable_auto_lock", True))
        self.enable_auto_lock_check.stateChanged.connect(self.on_auto_lock_toggle)
        lock_layout.addWidget(self.enable_auto_lock_check)
        ...
        # 初始化时根据设置控制闲置时间输入框的状态
        self.on_auto_lock_toggle()
    ...
    def on_auto_lock_toggle(self):
        """自动锁定开关状态改变时的处理"""
        enabled = self.enable_auto_lock_check.isChecked()
        # 当禁用自动锁定时，将闲置时间输入框设为不可用
        self.idle_spinbox.setEnabled(enabled)
    ...
    def verify_email(self):
        ...
        if not re.match(email_pattern, email):
            msg_box = QMessageBox(self)
            msg_box.setWindowTitle("警告")
            msg_box.setText("邮箱格式不正确，请输入有效的邮箱地址")
            msg_box.setIcon(QMessageBox.Icon.Warning)
            msg_box.addButton("确定", QMessageBox.ButtonRole.AcceptRole)
            msg_box.exec()
            return
...
"""
    ...
    def accept(self):
        """保存设置并关闭对话框"""
        # 更新设置
        self.settings["enable_auto_lock"] = self.enable_auto_lock_check.isChecked()
