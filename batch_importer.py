...
class BatchImporter:
    ...
    def get_preview_data(self, file_path: str, max_preview: int = None) -> tuple[list, list, int]:
        """获取导入预览数据
        
        Args:
            file_path: 文件路径
            max_preview: 最大预览行数（None表示不限制，显示全部）
        ...
        if max_preview is None:
            return valid_entries, invalid_entries, len(valid_entries)
